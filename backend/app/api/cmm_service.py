...
async def review(req: ReviewRequest, user_id: str = Depends(get_current_user)):
    try:
        ...
        user = User(user_id=user_id, theta={})
        card = Card(
            card_id=req.card.card_id or "tmp",
            beta=req.card.beta,
            c=req.card.c,
            review_count=req.card.review_count,
        )
        state = UserCardState(
            S=req.state.S,
            last_review_time=(req.state.last_review_time_hours or now - 24.0),
        )

        out = model.review(
            user, card, state, req.subject, req.actual_score, now_hours=now
        )
